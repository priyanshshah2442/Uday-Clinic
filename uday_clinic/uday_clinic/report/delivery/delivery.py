...
def get_columns(filters):
	...
	if filters.group_by == "Caste":
		fields = frappe.get_all("Caste", pluck="name", order_by="creation")
	else:
		fields = frappe.get_all("Delivery Types", pluck="name", order_by="creation")
...
def _get_data(filters):
	...
	query = (
		...
		.select(
			...
			INCOME_RECORDER_ITEMS.caste,
			Extract("month", INCOME_RECORDER.date).as_("month"),
			Extract("year", INCOME_RECORDER.date).as_("year"),
			Sum(IfNull(INCOME_RECORDER_ITEMS.number, 0)).as_("number_of_children"),
		)
		.groupby(
		...
		.where(INCOME_RECORDER_ITEMS.sub_type == "Delivery")
	...
	if filters.group_by == "Caste":
		query = query.groupby(INCOME_RECORDER_ITEMS.caste)
	if filters.group_by == "Method":
...
def process_result(result, filters):
	...
	for row in result:
		...
		column_name = (
			frappe.scrub(row.method.lower())
			if filters.group_by == "Method"
			else frappe.scrub(row.caste.lower())
		)

		data[(month, year)].update(
			{
				column_name: row.number_of_children,
				"total": data[(month, year)]["total"] + row.get("number_of_children", 0),
			}
		)
